/* Glitch Source: https://codepen.io/tksiiii/pen/xdQgJX */
/* Image to Base64 Source: https://davidwalsh.name/convert-image-data-uri-javascript */

var w = document.querySelector('#canvas__wrapper');
var windowW = w.getBoundingClientRect().width;
var windowH = w.getBoundingClientRect().height;
let isLoaded = false;
let glitch;
var imgSrc;

window.addEventListener("resize", function(){
	windowW = w.getBoundingClientRect().width;
	windowH = w.getBoundingClientRect().height;
	setup();
	draw();
});

getDataUri('/assets/images/gsheroblur-360.jpg', function (dataUri) {
	imgSrc = dataUri;
});

function setup() {
	background(0);
	createCanvas(windowW, windowH).parent('canvas__wrapper');
...
class Glitch {
	...
	replaceData(destImg, srcPixels) {
		for (let y = 0; y < destImg.height / 100; y++) {
			for (let x = 0; x < destImg.width / 100; x++) {
				let r, g, b, a;
				let index;
